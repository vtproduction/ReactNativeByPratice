...
export class MessagingContainer extends React.Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (!this.props.keyboardVisible && nextProps.keyboardVisible) { // Keyboard shown onChangeInputMethod(INPUT_METHOD.KEYBOARD);
		} else if (
			// Keyboard hidden
		...
		this.subscription = BackHandler.addEventListener(
			...
			() => {
				...
				if (inputMethod === INPUT_METHOD.CUSTOM) {
					onChangeInputMethod(INPUT_METHOD.NONE); return true;
				}
				return false;
			},
		);
